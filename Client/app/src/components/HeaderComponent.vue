      ...
      <div class="action-buttons">
        ...
        >
          <button class="action-button favorite-button">
            <font-awesome-icon
              icon="star"
              :class="['favorite-icon', { favorited: hasFavorites }]"
            />
          </button>
    ...
    async handleFavoriteHover() {
      ...
      try {
        ...
        if (response.data.length) {
          ...
          this.hasFavorites = true; // Define que há favoritos
          this.authenticated = true;
        } else {
          this.favorite_products = [];
          this.hasFavorites = false; // Não há favoritos
        }
      } catch (error) {
    ...
    async deleteProduct(productId) {
      try {
        ...
        if (response.status === 200) {
          ...
          this.hasFavorites = this.favorite_products.length > 0; // Atualiza o estado de favoritos
...
.favorite-icon {
  width: 2rem;
  height: 2rem;
  color: #cccccc; /* Cor padrão do ícone */
  transition: color 0.3s;
}

.favorite-icon.favorited {
  color: #e4bf5f; /* Cor quando há favoritos */
